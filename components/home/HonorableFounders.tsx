...
import { ChevronLeft, ChevronRight, Award, ZoomIn, Users, Star } from 'lucide-react'
...
const HonorableFounders = () => {
  ...
  return (
    <>
      <section className="py-12 sm:py-16 lg:py-20 bg-gradient-to-br from-indigo-50 via-primary-50 to-accent-50 relative overflow-hidden">
        ...
        <div className="container-custom relative z-10">
          ...
          <div className="text-center mb-10 sm:mb-12 lg:mb-16">
            <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-indigo-500 to-indigo-600 rounded-full px-6 sm:px-8 py-2 sm:py-3 mb-6 shadow-lg">
              <Users className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
              <span className="text-sm sm:text-base md:text-lg font-bold text-white">Leadership Team</span>
            </div>
          ...
          <div className="relative max-w-7xl mx-auto">
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden border-4 border-indigo-300/30">
              <div className="relative min-h-[600px] sm:min-h-[700px] overflow-hidden bg-gradient-to-br from-indigo-50/50 to-primary-50/50">
                    ...
                    <div className="h-full flex items-center justify-center p-6 sm:p-8 lg:p-12">
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-10 w-full max-w-6xl">
                        ...
                        <div className="group relative order-2 lg:order-1">
                          <div className="relative h-[450px] sm:h-[550px] lg:h-[600px] rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl cursor-pointer" onClick={() => setZoomedImage(founder.image)}>
                            <Image
                              src={founder.image}
                              alt={founder.name}
                              fill
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex items-end p-6 sm:p-8">
                              <div className="text-white w-full">
                                <div className="flex items-center space-x-2 mb-2">
                                  ...
                                  <span className="text-sm font-medium text-indigo-300">{founder.title}</span>
                                </div>
                            ...
                            <div className="absolute top-4 left-4 bg-indigo-500 text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg">
                              👥 Leadership
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="order-1 lg:order-2 flex flex-col justify-center space-y-6">
                          <div className="bg-white/95 backdrop-blur-sm rounded-xl sm:rounded-2xl p-6 sm:p-8 shadow-xl">
                            <div className="space-y-6">
                              <div className="p-4 bg-gradient-to-r from-indigo-100 to-primary-100 rounded-lg border-l-4 border-indigo-500">
                                <p className="text-lg font-bold text-secondary-800 break-words">{founder.title}</p>
                              </div>
                              ...
                              <div className="space-y-3">
                                <p className="text-sm font-semibold text-secondary-600 mb-2">Key Achievements:</p>
                                {founder.achievements.map((achievement, idx) => (
                                  <div key={idx} className="flex items-start space-x-3 p-3 bg-gradient-to-r from-indigo-50/50 to-primary-50/50 rounded-lg">
                                    <Star className="w-5 h-5 text-indigo-600 flex-shrink-0 mt-0.5" />
                                    <span className="text-sm text-secondary-700 font-medium break-words">{achievement}</span>
                                  </div>
                                ))}
                              </div>
                              
                              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                                <div className="flex items-center space-x-2 bg-indigo-100 rounded-full px-4 py-2 flex-1 justify-center">
                                  <Award className="w-5 h-5 text-indigo-600" />
                                  <span className="text-sm font-semibold text-indigo-700">Educational Leadership</span>
                                </div>
                                <div className="flex items-center space-x-2 bg-accent-100 rounded-full px-4 py-2 flex-1 justify-center">
                                  ...
                                  <span className="text-sm font-semibold text-accent-700">Visionary Leadership</span>
                                </div>
                              </div>
                            </div>
                          </div>
              ...
              <button
                onClick={prevSlide}
                className="absolute left-4 sm:left-6 top-1/2 -translate-y-1/2 w-12 h-12 sm:w-14 sm:h-14 bg-white hover:bg-indigo-50 active:bg-indigo-100 text-secondary-700 hover:text-indigo-600 rounded-full flex items-center justify-center shadow-xl hover:shadow-2xl transition-all duration-300 z-20 hover:scale-110 active:scale-95 touch-target min-h-[44px] min-w-[44px] border-2 border-indigo-200"
              ...
              <button
                onClick={nextSlide}
                className="absolute right-4 sm:right-6 top-1/2 -translate-y-1/2 w-12 h-12 sm:w-14 sm:h-14 bg-white hover:bg-indigo-50 active:bg-indigo-100 text-secondary-700 hover:text-indigo-600 rounded-full flex items-center justify-center shadow-xl hover:shadow-2xl transition-all duration-300 z-20 hover:scale-110 active:scale-95 touch-target min-h-[44px] min-w-[44px] border-2 border-indigo-200"
              ...
              <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-lg z-20">
                <span className="text-sm font-semibold text-secondary-700">
                  {currentSlide + 1} / {founders.length}
                </span>
              </div>
            </div>
          </div>

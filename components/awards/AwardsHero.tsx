'use client'

import { Trophy, Award, Star, Medal } from 'lucide-react'

const AwardsHero = () => {
  const awardCategories = [
    {
      icon: <Trophy className="w-8 h-8" />,
      title: 'Academic Excellence',
      description: 'Outstanding academic performance awards'
    },
    {
      icon: <Award className="w-8 h-8" />,
      title: 'Sports Achievements',
      description: 'Recognition for sports and athletics'
    },
    {
      icon: <Star className="w-8 h-8" />,
      title: 'Leadership',
      description: 'Student leadership and service awards'
    },
    {
      icon: <Medal className="w-8 h-8" />,
      title: 'Special Recognition',
      description: 'Unique achievements and contributions'
    }
  ]

  return (
    <section className="relative min-h-screen flex items-center bg-gradient-to-br from-primary-600 via-primary-700 to-accent-600 text-white overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute inset-0 bg-[url('/images/pattern.svg')] bg-repeat"></div>
      </div>

      <div className="container-custom relative z-10 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          {/* Content */}
          <div className="space-y-8">
            <div className="space-y-6">
              <h1 className="text-4xl md:text-6xl font-bold font-josefin leading-tight">
                <span className="text-gradient">Awards & Recognition</span>
              </h1>
              <p className="text-xl md:text-2xl text-white/90 leading-relaxed">
                Celebrating Excellence and Achievement
              </p>
              <p className="text-lg text-white/80 leading-relaxed">
                At Pak Wattan School & College of Sciences, we believe in recognizing 
                and celebrating the achievements of our students, faculty, and community. 
                Our awards program honors excellence in academics, sports, leadership, and service.
              </p>
            </div>

            <div className="flex flex-col sm:flex-row gap-4">
              <button className="btn-accent text-center">
                View Gallery
              </button>
              <button className="btn-secondary text-center">
                Learn More
              </button>
            </div>
          </div>

          {/* Award Categories Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {awardCategories.map((category, index) => (
              <div
                key={index}
                className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 hover:bg-white/20 transition-all duration-300 group"
              >
                <div className="text-white mb-4 group-hover:scale-110 transition-transform duration-300">
                  {category.icon}
                </div>
                <h3 className="text-lg font-semibold text-white mb-2">
                  {category.title}
                </h3>
                <p className="text-white/80 leading-relaxed">
                  {category.description}
                </p>
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* Floating Elements */}
      <div className="absolute top-20 right-20 w-20 h-20 bg-white/10 rounded-full animate-bounce-slow"></div>
      <div className="absolute bottom-20 left-20 w-16 h-16 bg-accent-500/20 rounded-full animate-bounce-slow delay-1000"></div>
    </section>
  )
}

export default AwardsHero
